process SPADES {
    ...
    tuple val(meta), path("*.assembly_graph_with_scaffolds.gfa"), emit: assembly_graph_with_scaffolds, optional: true
    tuple val(meta), path("*.assembly_graph_after_simplification.gfa"), emit: assembly_graph_after_simplification, optional: true
    ...
    def maxmem = task.memory ? task.memory.toGiga() : 16
    ...
    if [ -f assembly_graph_after_simplification.gfa ]; then
        mv assembly_graph_after_simplification.gfa ${prefix}.assembly_graph_after_simplification.gfa
        gzip -n ${prefix}.assembly_graph_after_simplification.gfa
    fi
